...
stop=["stop","Stop","STOPPED","End","terminate","stoping","stopping"]
...
error=input("Enter the issue or service name do you want to stop:")
...
if any(word in error for word in dependant_service) or any(word in error for word in dependant_displayname) :
    ...
    continuecheck = input("Do you wish to Stop this service?(yes/no)")

    if continuecheck == "yes":
        print("Please wait!", IssuedService, "Will stop")
    ...
    error=input("Enter the exact problem or service name  you want to stop: ")
    if any(word in error for word in dependant_service) or any(word in error for word in dependant_displayname):
        Error_List = error.split()
        if any(word in error for word in dependant_service):
        ...
        continuecheck = input("Do you wish to Stop the service?(yes/no)")
        if continuecheck == "yes":
            exec(open("Powershell/StopService.py").read())
            print("Please wait!", IssuedService, "Will stop")
            sys.exit()
            ...
            print("Wrong Input.Thank you for joining with SAITA")
            exit()
